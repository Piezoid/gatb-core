...
public:
    ...
    BankAlbum (const std::string& name, const IBank::vector& banks)
    ...
    IBank::sptr addBank (const std::string& bankUri);

    /** Add a bank to the album. */
    IBank::sptr addBank (const std::string& directory, const std::string& bankName, bool output_fastq=false, bool output_gz=false);
